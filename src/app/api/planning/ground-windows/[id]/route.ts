...
export async function PATCH(request: NextRequest, { params }: Params) {
  try {
    ...
    const window = await aircraftPlanningService.updateGroundWindow(supabase, id, values);
...
export async function DELETE(_request: NextRequest, { params }: Params) {
  try {
    const currentUser = await requireCurrentUser();
    if (currentUser.role !== "Admin") throw new PermissionError("只有管理員可以刪除地面時間紀錄");
    const { id } = await params;
    const supabase = await createClient();
    await aircraftPlanningService.deleteGroundWindow(supabase, id);
